	...
	for node1 in graph.vs:
		for node2 in graph.vs:
			if(actualClusters.membership[node1.index] == actualClusters.membership[node2.index] and newClusters.membership[node1.index] == newClusters.membership[node2.index]):
				...
				count = count + 1
			if(actualClusters.membership[node1.index] == actualClusters.membership[node2.index] and originalClusters.membership[node1.index] == originalClusters.membership[node2.index]):
				count1 = count1 + 1
			elif(actualClusters.membership[node1.index] != actualClusters.membership[node2.index] and originalClusters.membership[node1.index] != originalClusters.membership[node2.index]):
				count1 = count1 + 1
			i = i + 1
	error = (float(count) / float(i))
	error1 = (float(count1) / float(i))
	print error
	print error1
